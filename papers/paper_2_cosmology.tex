...
\subsection{The Surface Brightness (Tolman) Test}
A critical test for any static cosmology is the surface brightness dimming law. In an expanding universe, surface brightness scales as $(1+z)^{-4}$. In a naive static universe, it scales as $(1+z)^{-1}$, which is ruled out by observations.
However, in the Isothermal Machian Universe, the "Universal Conformal Coupling" ensures agreement with the standard $(1+z)^{-4}$ law.
The dimming factors arise as follows:
\begin{enumerate}
    \item \textbf{Energy Dimming:} $E_{obs} = E_{emit}/(1+z)$ (derived in Section 2.2).
    \item \textbf{Time Dilation:} Arrival rates scale as $dt_{obs} = dt_{emit}(1+z)$ due to the mass-dependent clock rates.
    \item \textbf{Conformal Aperture:} The detector area $A$ is defined by atomic rulers ($A \propto a_0^2$). Since mass evolves, the "size" of the detector relative to the conformal metric changes. As proven in Paper 5, the metric $\tilde{g}_{\mu\nu}$ is conformally dual to FLRW. Since observables are conformally invariant, the flux per unit physical area must scale identically to the expanding frame prediction: $F \propto (1+z)^{-4}$.
\end{enumerate}
Thus, the IMU passes the Tolman surface brightness test. The choice between "Expansion" and "Mass Evolution" is therefore a choice of coordinate frame, not empirical fact. However, as noted in the discussion, the Static Frame offers a natural resolution to the "Early Galaxy" time-scale problem.

...
Mimetic Gravity is known to suffer from potential gradient instabilities (caustics) or "ghost" modes when the sound speed vanishes ($c_s^2 = 0$). In our framework, we interpret the theory as an Effective Field Theory (EFT) valid up to a cutoff scale $\Lambda_{UV}$.
We note that the higher-derivative regulator $(\Box \phi)^2$ technically introduces an Ostrogradsky ghost degree of freedom. However, in the spirit of Effective Field Theory, this ghost is massive with $m_{ghost} \sim \Lambda_{UV}$. Since we treat the scalar sector as a superfluid condensate valid only below the UV cutoff, this mode is integrated out and does not affect the causal structure of the low-energy theory. In the hydrodynamic limit, it manifests purely as the non-dynamical Bohm quantum potential.
Above $\Lambda_{UV}$, the theory requires a full UV completion (e.g., String Theory).

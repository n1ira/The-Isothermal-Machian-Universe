...
\end{equation}
Since gravitational mass $m_g$ and inertial mass $m(r)$ are distinct in this theory ($m_g$ is constant, $m(r)$ drops), we get:
\begin{equation}
    v^2 = \frac{G M(r)}{r} \left( \frac{m_g}{m(r)} \right) \propto \frac{1}{r} \cdot \frac{1}{e^{-r/R}} \approx \text{const}
\end{equation}
...
We performed a high-resolution N-body simulation (Experiment 8) using a \textbf{P3M (Particle-Particle Particle-Mesh)} code to resolve the small-scale clustering dynamics. The simulation evolved $64^3$ particles from $z=50$ to $z=0$ under the influence of Newtonian gravity and the Machian scalar force.
...
In our framework, the scalar field couples to the Higgs sector via a dimension-5 operator:
\begin{equation}
